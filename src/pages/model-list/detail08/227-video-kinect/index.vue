<script setup lang="ts">
  import Page from "@/base/page/index.vue";
</script>

<template>
  <div class="webgl-video-kinect-page">
    <Page title="webgl-video-kinect">
      <div ref="container" class="key-frame-page-inner">
        <video ref="video" loop muted crossOrigin="anonymous" playsinline style="display:none">
          <source src="/examples/textures/kinect.webm">
          <source src="/examples/textures/kinect.mp4">
        </video>
      </div>
    </Page>
  </div>
</template>

<script lang="ts">
import { defineComponent } from "vue";
import { Model } from "./tools";

let objModel: Model | null;
export default defineComponent({
  data() {
    return {
      show: false,
    };
  },
  mounted() {
    this.render();
  },
  methods: {
    // 渲染入口
    render() {
      const container = this.$refs.container as HTMLDivElement;
      const video = this.$refs.video as HTMLVideoElement;
      objModel = new Model(container, video);
      objModel.init();
    },
  },
  // 卸载前函数
  beforeUnmount() {
    objModel?.dispose();
    objModel = null;
  }
});
</script>
<style lang='less'>
  @import "@/common/style/color.less";
  @import "@/common/style/mixins.less";

  .webgl-video-kinect-page {
    .absolute-page();
    background-color: #000;
    .key-frame-page-inner {
      position: relative;
      .width-and-height();
      .lil-gui.root {
        max-height: 50%;
        max-width: 80%;
        position: absolute;
        top: auto;
        bottom: 0;
        left: 0;
        right: auto;
      }
    }
  }
</style>
