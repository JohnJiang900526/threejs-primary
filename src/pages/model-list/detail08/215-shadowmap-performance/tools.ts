...
export class Model {
  ...
  private readonly SHADOW_MAP_WIDTH: number;
  private readonly SHADOW_MAP_HEIGHT: number;
  private readonly FLOOR: number;
  private readonly NEAR: number;
  private readonly FAR: number;
  private readonly ANIMATION_GROUPS: number;
  private mixer: null | THREE.AnimationMixer;
  private morphs: THREE.Mesh[];
  private animGroups: THREE.AnimationObjectGroup[];
  ...
  private createScene () {
    ...
    for (let i = 0; i < this.ANIMATION_GROUPS; i++) {
      // 接收共享动画状态的一组对象。

      // 将本来要作为根对象传入构造器或者动画混合器(AnimationMixer)的clipAction方法中的对象加入组中，
      // 并将这个组对象作为根对象传递。
    ...
    const addMorph = (Mesh: THREE.Mesh, clip: THREE.AnimationClip, speed: number, duration: number, x: number, y: number, z: number, fudgeColor: boolean = false, massOptimization: boolean = false) => {
      ...
      if (fudgeColor) {
        ...
        material.color.offsetHSL(h, s, l);
      }
      ...
      if (massOptimization) {
        const index = Math.floor(Math.random() * this.ANIMATION_GROUPS);
				const group = this.animGroups[index];

        group.add(mesh);

        const notExisting = !this.mixer?.existingAction(clip, group);
        if (notExisting) {
          const randomness = 0.6 * Math.random() - 0.3;
					const phase = (index + randomness) / this.ANIMATION_GROUPS;

          this.mixer?.clipAction(clip, group).setDuration(duration).startAt(-duration * phase).play();
        }
      } else {
