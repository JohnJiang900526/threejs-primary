...
<template>
  <div class="webgl-interactive-voxel-painter-page">
    <Page title="webgl-interactive-voxel-painter">
    ...
    <Popup :show="show" position="right" @click-overlay="closeHandle" :style="{ height: '100%', width: '80%' }">
      <Page @click-left="closeHandle" :default-click="false" title="模型设置">
        <div class="settings-content">
          <Block title="操作">
            <Space size="2rem">  
              <Button @click="deleteHandle" size="small" type="primary">逐个删除</Button>
              <Button @click="clearHandle" size="small" type="primary">全部删除</Button>
            </Space>
          </Block>
        </div>
      </Page>
...
export default defineComponent({
  ...
  methods: {
    ...
    deleteHandle() {
      if (objModel) {
        this.closeHandle();
        objModel.deleteHandle();
      }
    },
    clearHandle() {
      if (objModel) {
        this.closeHandle();
        objModel.clearHandle();
      }
    },
    // 打开设置面板
...
<style lang='less' scoped>
  ...
  .webgl-interactive-voxel-painter-page {
    .absolute-page();
